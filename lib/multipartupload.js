...
MultiPartUpload.prototype._handleStream = function(stream, callback) {
    ...
    function newPart() {
      // Create a new part
        var object = mpu.objectName.replace(/\//g, '-');
        var partId = parts.length + 1,
            partFileName = path.resolve(path.join(mpu.tmpDir || '', 'mpu-' + object + '-' + random_seed() + '-' + (mpu.uploadId || Date.now()) + '-' + partId)),
...
MultiPartUpload.prototype._uploadPart = function(part, callback) {
    ...
    req.on('response', function(res) {
        if (res.statusCode != 200) {
            ...
            if (--part.triesLeft) {
              this.snsClient && this.snsClient.publish({
                Message: 'MPU part failed to upload: ' + part.fileName,
        ...
        if(fs.existsSync(part.fileName)) {
          fs.unlink(part.fileName, function(err) {
            if(err) {
              this.snsClient && this.snsClient.publish({
                Message: 'MPU part failed to delete: ' + part.fileName,
                TopicArn: this.snsTopic
              }, function(err, data) {
                if(err) mpu.emit('failed', err);
              });
              mpu.emit('failed', err);
            }
          });
    ...
    mpu.emit('uploading', { id: part.id, filename: part.fileName});
};
