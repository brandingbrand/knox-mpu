...
MultiPartUpload.prototype._handleStream = function(stream, callback) {
    ...
    // Create a new part
    var partObjectName = mpu.objectName.replace(/\//g, '-');
    function newPart() {
        var partId = parts.length + 1,
            partFileName = path.resolve(path.join(mpu.tmpDir || '', 'mpu-' + partObjectName + '-' + random_seed() + '-' + (mpu.uploadId || Date.now()) + '-' + partId)),
        ...
        console.info('New part: ', partFileName);
        parts.push(part);
        return part;
    }
